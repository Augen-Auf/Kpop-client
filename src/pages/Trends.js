    ...
    return (
        <div className="py-10 px-4">
            <div className="max-w-4xl mx-auto flex flex-col items-center font-montserrat font-normal text-black">
                { topNew  && pageNumber === 0 &&
                <div
                    className="bg-pink w-full h-96 flex flex-col justify-end py-5 mb-10 rounded-md cursor-pointer"
                    onClick={() => history.push('/news/'+topNew.id)}
                ...
                <div className="w-full space-y-3">
                    {
                        news.length > 0 && news.slice(elementsVisited, elementsVisited + elementsPerPage).map(item =>
                            <div className="w-full bg-yellow flex rounded-md p-3 justify-between cursor-pointer"
